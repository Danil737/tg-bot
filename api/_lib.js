...
const YANDEX_REVIEW = 'https://yandex.ru/maps/org/ukhodmogil/11946621760/reviews/?add-review=1'
...
function graveOwnerText(g, origin = 'по сообщению клиента') {
  ...
  const head = `🔎 <b>${htmlEscape(g.cemetery || 'Реестр захоронений')}</b> · <i>${htmlEscape(origin)}</i>` +
    ...
    (g.via_text
      // via_text с сервера уже начинается с «найдено …» — не дублируем слово, только
      // делаем заглавную. Если формулировка иная — префикс «Найдено» остаётся.
      ? `\n⚠️ <i>${htmlEscape(/^найдено/i.test(g.via_text.trim())
          ? g.via_text.trim().charAt(0).toUpperCase() + g.via_text.trim().slice(1)
          : 'Найдено ' + g.via_text)}.</i>`
      : (g.relaxed
...
function graveClientText(r) {
  ...
  return '🕯 Нашли в открытом реестре захоронений:\n\n' +
    ...
    // Просьба об отзыве идёт ПОСЛЕ помощи и без условий: «поставьте оценку — тогда
    // дам координаты» превращает бесплатную подсказку в обмен данных на выгоду, а так
    // это обычная благодарность. Ссылка проверена 26.08.2026: yandex.ru/profile/org/...
    // отдаёт 404, рабочая — карточка в Картах с открытой формой отзыва.
    `

Подсказка бесплатная. Если помогла — нам очень поможет короткий отзыв: ${YANDEX_REVIEW}`
}
...
// Сообщение, которое целиком состоит из ФИО (часто с датой), — это тоже запрос.
// 07.08 вторым сообщением пришло «Рябченко Лилия Дмитриевна / 03.03.2003»: слова
// «кладбище» в нём уже не было, потому что оно было в первом, — и бот промолчал.
const FIO_ONLY = /^[\s\d.,–—-]*[А-ЯЁ][а-яё-]{2,}\s+[А-ЯЁ][а-яё-]{2,}(?:\s+[А-ЯЁ][а-яё-]{2,})?[\s\d.,–—-]*$/
