...
for (const [t, want] of [
  ...
  ['кладбище', false],
  // 07.08: вторым сообщением владелец прислал ОДНО ФИО с датой — ни «кладбища»,
  // ни «могилы» в нём нет, и бот промолчал, хотя это и был весь запрос.
  ['Рябченко Лилия Дмитриевна\n03.03.2003', true],
  ['Колычев Анатолий Иванович', true],
  ['Морозова Мария Фёдоровна, 1930', true],
  ['Сегодня отправил отчёт', false],
]) {
  check('триггер: ' + (want ? 'ищем' : 'молчим') + ' — ' + t.slice(0, 32),
    looksLikeGraveText(t) === want)
}

// Нашли по фамилии, потому что по полному ФИО реестр пуст. Владелец обязан видеть,
// что перед ним однофамилец: иначе он подтвердит клиенту чужую могилу.
const relaxed = graveOwnerText(
  { ...VAGUE, fio: 'Рябченко Лилия Дмитриевна', relaxed: 'Рябченко' }, 'по сообщению клиента')
check('сказано, что найдено по фамилии, а не по полному ФИО',
  relaxed.includes('Рябченко Лилия Дмитриевна') && relaxed.includes('однофамильцы'))
check('без отступления оговорки нет',
  !graveOwnerText(VAGUE, 'по сообщению клиента').includes('однофамильцы'))
...
// Поиск идёт ступенями, и ступень меняет смысл найденного. 07.08.2026 могилу нашли
// только маской: на памятнике «Бялясинский», в реестре «Балясинский». Такое совпадение
// показывать молча нельзя — но и прятать тоже, иначе поиск бесполезен.
const masked = graveOwnerText(
  { ...VAGUE, fio: 'Бялясинский Серёженька',
    via_text: 'по маске «*лясинск*» — в реестре фамилия записана с другой буквой' })
check('сказано, что нашли по маске из-за опечатки в реестре',
  masked.includes('по маске') && masked.includes('другой буквой'))

// Один реестр ответил, второй лёг: «не нашли» и «не искали» — разные вещи.
const partial = graveOwnerText({ ...VAGUE, registry_errors: { mo: 'error: таймаут' } })
check('назван реестр, который не ответил',
  partial.includes('Не ответил') && partial.includes('реестр Подмосковья'))
check('без сбоя про реестры молчим', !graveOwnerText(VAGUE).includes('Не ответил'))

// Запись из подмосковного реестра: место словами, свои координаты, свой источник.
const MO_ROW = {
  gid: '557211250', uchastok: 'сектор 6, ряд 6, место 2158',
  cemetery: 'Ногинское городское кладбище № 1', source: 'mo',
  inscription: ['Носко Мария Савельевна, 1915-04-11 – 2004-01-25'],
  lat: 55.89135876, lon: 38.4352, map_url: 'https://yandex.ru/maps/?ll=38.4352,55.89135876',
  score: 0, why: [],
}
const moText = graveOwnerText({ ok: true, cemetery: 'Ногинское', results: [MO_ROW] })
check('место из областного реестра показано как есть', moText.includes('сектор 6, ряд 6, место 2158'))
check('видно, из какого реестра запись', moText.includes('реестр Подмосковья'))
check('координаты видны текстом, чтобы переслать мастеру', moText.includes('55.89135876, 38.4352'))
...
// --- сбой реестра != пустая выдача ---------------------------------------
// epoisk отдаёт ошибку молча: пустой список + source «error: ...». Раньше это
// печаталось как «в реестре ничего не нашлось», и владелец делал ровно обратный
// вывод — что записи о могиле нет.
const src = require('fs').readFileSync(require('path').join(__dirname, '../api/webhook.js'), 'utf8')
check('бот разводит сбой реестра и пустую выдачу', src.includes('g.degraded'))
check('при сбое сказано, что поиск НЕ выполнен',
  /Реестр захоронений не ответил[\s\S]{0,120}поиск НЕ выполнен/.test(src))
check('при сбое НЕ звучит «реестр открытый и неполный»',
  src.indexOf('g.degraded') < src.indexOf('Реестр открытый и неполный'))

// --- ответ владельца не теряется молча ------------------------------------
// chatStore.message при сбое возвращает null, а не бросает. Раньше следом
// безусловно ставилась ✅ — владелец был уверен, что клиент получил ответ.
check('запись ответа владельца проверяется', /const saved = await chatStore\.message/.test(src))
check('при незаписи владельцу говорят прямо', src.includes('Ответ НЕ сохранился'))
check('при незаписи галка ✅ не ставится',
